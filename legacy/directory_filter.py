#!/usr/bin/env python3
"""
Directory Filter Example for GitHub PR Analyzer

This script demonstrates how to modify the GitHub PR Analyzer to filter PRs by specific directories.
In this example, we'll find PRs with 2+ file changes merged after November 2024 that include
changes to files in specific directories or code areas.
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Find GitHub PRs with changes in specific directories')
    ...
    parser.add_argument('--directories', help='Comma-separated list of directories to filter by (e.g., "src,docs,tests")',
                        required=True)
...
def main():
    ...
    print(f"Analyzing pull requests (directories: {', '.join(directories)})...")
    for pr in all_prs:
        pr_number = pr['number']
        ...
        # Check if PR includes changes to specified directories
        matching_files = []
        for file in files:
        ...
        # Only include PR if it has changes to specified directories
        if matching_files:
            filtered_prs.append({
                'number': pr_number,
                'title': pr['title'],
    ...
    print(f"that include changes to directories: {', '.join(directories)}")
    print("\n" + "-" * 80)
