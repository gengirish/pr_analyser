#!/usr/bin/env python3
"""
Custom Date Filter Example for GitHub PR Analyzer

This script demonstrates how to modify the GitHub PR Analyzer to use a custom date filter.
In this example, we'll find PRs with 2+ file changes merged after a custom date
that can be specified as a command-line argument.
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Find GitHub PRs with 2+ file changes merged after a custom date')
...
def main():
    ...
    print(f"Analyzing pull requests (date filter: {args.date})...")
    for pr in all_prs:
        pr_number = pr['number']
    ...
    # Display results
    print("\nResults:")
    print(f"Found {len(filtered_prs)} PRs with 2+ file changes merged after {args.date}:")
    print("\n" + "-" * 80)
